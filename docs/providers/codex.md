...
- both accounts can see the same Androdex/Codex sessions
...
In Androdex Settings, name it something obvious:
...
In Androdex Settings, add another Codex provider:
...
Androdex shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Androdex treats it
