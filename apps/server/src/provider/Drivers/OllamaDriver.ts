...
export type OllamaDriverEnv = HttpClient.HttpClient;
...
export const OllamaDriver: ProviderDriver<OllamaSettings, OllamaDriverEnv> = {
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      const maintenanceCapabilities: ProviderMaintenanceCapabilities = {
      ...
      const snapshot = yield* makeManagedServerProvider<OllamaSettings>({
        ...
        initialSnapshot: (settings) =>
          makePendingOllamaProvider(settings).pipe(Effect.map(stampIdentity)),
        checkProvider,
