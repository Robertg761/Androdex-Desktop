...
function SheetBackdrop({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      className={cn(
        "fixed inset-0 z-50 bg-black/32 backdrop-blur-[8px] transition-all duration-200 data-ending-style:opacity-0 data-starting-style:opacity-0",
        className,
      )}
    ...
    <SheetPortal keepMounted={keepMounted}>
      ...
      <SheetViewport side={side} variant={variant}>
        <SheetPrimitive.Popup
          className={cn(
            "liquid-glass-surface liquid-glass-surface-floating relative flex max-h-full min-h-0 w-full min-w-0 flex-col bg-popover not-dark:bg-clip-padding text-popover-foreground shadow-[0_20px_70px_rgb(0_0_0_/_0.16)] transition-[opacity,translate] duration-200 ease-in-out will-change-transform before:pointer-events-none before:absolute before:inset-0 before:shadow-[0_1px_--theme(--color-black/3%)] data-ending-style:opacity-0 data-starting-style:opacity-0 max-sm:before:hidden dark:before:shadow-[0_-1px_--theme(--color-white/5%)]",
            side === "bottom" &&
            ...
            variant === "inset" &&
              "before:hidden sm:rounded-lg sm:border sm:before:rounded-[calc(var(--radius-lg)-1px)] sm:**:data-[slot=sheet-footer]:rounded-b-[calc(var(--radius-lg)-1px)]",
            className,
          )}
