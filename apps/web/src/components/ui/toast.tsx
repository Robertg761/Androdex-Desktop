...
const toastCornerOrbClass = cn(
  "liquid-glass-control inline-flex size-6 shrink-0 cursor-pointer items-center justify-center rounded-full border border-border/60 bg-popover/92 text-muted-foreground shadow-sm outline-none backdrop-blur-sm",
      ...
      >
        {visibleToastLayout.items.map(({ toast, visibleIndex, offsetY }) => {
          ...
          return (
            <Toast.Root
              className={cn(
                "liquid-glass-surface liquid-glass-surface-floating absolute z-[calc(9999-var(--toast-index))] w-full overflow-visible select-none rounded-lg border bg-popover not-dark:bg-clip-padding text-popover-foreground shadow-[0_12px_40px_rgb(0_0_0_/_0.12)] [transition:transform_.5s_cubic-bezier(.22,1,.36,1),opacity_.5s,height_.15s] before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] before:shadow-[0_1px_--theme(--color-black/3%)] dark:before:shadow-[0_-1px_--theme(--color-white/5%)]",
